...
export default function Login() {
  ...
  const go = async () => { const r = await fetch("/api/auth/login", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ email, password: pw }) }); if (r.ok) location.href = "/admin"; else setErr((await r.json().catch(() => ({}))).error === undefined ? "Wrong email or password." : "Wrong email or password."); };
  return (
    <div className="grid min-h-screen place-items-center px-4">
      <div className="w-full max-w-sm">
        <div className="eyebrow mb-1">Point Radius</div>
