...
export async function POST(req: NextRequest) {
  ...
  if (action === "approve") {
    await q(`UPDATE assets SET visibility='visible', moderated_at=now(), moderated_by=$2
             WHERE id = ANY($1::uuid[]) AND status='ready' AND visibility='pending'`, [assetIds, user.id]);
    await audit(user.id, "approve", { count: assetIds.length }, iph);
    return NextResponse.json({ approved: assetIds.length });
  }
  if (action === "reject") {
    await q(`UPDATE assets SET visibility='rejected', moderated_at=now(), moderated_by=$2 WHERE id = ANY($1::uuid[])`, [assetIds, user.id]);
    await audit(user.id, "reject", { count: assetIds.length }, iph);
    return NextResponse.json({ rejected: assetIds.length });
  }
  if (action === "requeue") {
    await q(`UPDATE assets SET visibility='pending' WHERE id = ANY($1::uuid[])`, [assetIds]);
