...
export default async function P({ params }: { params: { token: string } }) {
  const [l] = await q(`SELECT l.mode, l.is_active, g.name, g.brand, g.upload_terms, g.allow_uploads
  ...
  return <Uploader token={params.token} mode={l.mode} galleryName={l.name} terms={l.upload_terms || ""} brand={{ primary: l.brand?.primary || "#E8442A", accent: l.brand?.accent || "#D6E04B" }} />;
}
